...
if [ "$with_mkl" != "__DONTUSE__" ] ; then
    ...
    mkl_required_libs="libmkl_gf_lp64.so libmkl_core.so libmkl_sequential.so"
    ...
    MKL_LIBS="-Wl,--start-group ${mkl_lib_dir}/libmkl_gf_lp64.so ${mkl_lib_dir}/libmkl_core.so ${mkl_lib_dir}/libmkl_sequential.so"
    ...
    if [ $MPI_MODE != no ] ; then
        ...
        case $MPI_MODE in
            mpich)
                mkl_optional_libs="$mkl_optional_libs libmkl_blacs_lp64.so"
                mkl_blacs_lib="libmkl_blacs_lp64.so"
                ;;
            openmpi)
